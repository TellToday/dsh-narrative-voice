 ...
 * touched.
 *
 * Live on/off: registers a `/voice` slash command (via the `commands`
 * service) that flips an in-process boolean. Because `assemble()` runs per
 * request, the change applies from the next message — no restart and no HMR
 * dependency (the web profile disables HMR).
 *
 * Default voice is "user" (方案B): the answerer narrates — "我" = the person
 * answering, "你" = the AI. Set `voice: "ai"` for the mirror (方案A).
 *
 ...
 */
function Config(raw = {}) {
	if (raw === null || typeof raw !== "object") throw new Error("narrative-voice: config must be an object");
...
const VOICE_USER_RULE = "Writing rule (this tool only): write the question, header, label, and description from the answerer's point of view — \"我\" is the person answering, \"你\" is the AI. The mapping is fixed: never swap them, and never call the answerer \"用户\" or any other third-person word. Keep each option self-consistent: the \"我\" in a label and the \"我\" in its description must be the same person, and the \"你\" likewise; a description may say what the AI will do (\"你\" will …) without shifting the label's perspective. If a draft drifts — say a label from the answerer's view but its description from the AI's view — rewrite the whole block rather than patching it. This rule only governs this tool; it does not change your ordinary reply voice.";

/** 方案A rule — the AI is the narrator. */
const VOICE_AI_RULE = "Writing rule (this tool only): write the question, header, label, and description from the AI's point of view — \"我\" is the AI, \"你\" is the person answering. The mapping is fixed: never swap them, and never call the answerer \"用户\" or any other third-person word. Keep each option self-consistent: the \"我\" in a label and the \"我\" in its description must be the same person, and the \"你\" likewise; a description may say what the answerer will do (\"你\" will …) without shifting the label's perspective. If a draft drifts — say a label from the AI's view but its description from the answerer's view — rewrite the whole block rather than patching it. This rule only governs this tool; it does not change your ordinary reply voice.";
...
function apply(ctx, config) {
	...
	ctx.commands.register({
		name: "voice",
		description: "toggle the ask_user_question narrative-voice rewrite (on|off)",
		input: { hint: "on|off" },
		recordInput: false,
		handler: (invocation) => {
			...
			else if (raw !== "") return { kind: "error", text: "Usage: /voice on|off" };
			return { kind: "success", text: `narrative-voice rewrite is ${active ? "ON" : "OFF"} for ask_user_question.` };
		}
	});
}
// #endregion
