 ...
 *
 * Default voice is "user" (方案B): the question and header are written from
 * the AI's point of view ("I" = the AI, "You" = the person answering), while
 * the options (label/description) are written from the answerer's point of
 * view. Set `voice: "ai"` for the uniform AI-narrated variant (方案A: "I" =
 * the AI in every field). The rule text is English-only (zero CJK), so it
 * cannot leak Chinese characters into English output.
 *
...
/** 方案B rule — question/header always AI-narrated ("I"=AI, "You"=the answerer); options (label/description) from the answerer's view. English-only (zero CJK). */
const VOICE_USER_RULE = "Writing rule (this tool only): write the question and header from the AI's point of view — \"I\" is the AI, \"You\" is the person answering (the user); this question/header mapping is fixed in both schemes and never changes. Write the label and description from the answerer's point of view — \"I\" is the person answering, \"You\" is the AI. Never swap either mapping, and never call the answerer \"the user\" or any other third-person word. Keep each option self-consistent: the \"I\" in a label and the \"I\" in its description must be the same person, and the \"You\" likewise; a description may say what the AI will do (\"You\" will …) without shifting the label's perspective. If a draft drifts — say a label from the answerer's view but its description from the AI's view — rewrite the whole block rather than patching it. This rule applies only where such pronouns actually appear — never force \"I\"/\"You\" into a question or option that does not naturally need them. This rule only governs this tool; it does not change your ordinary reply voice.";

/** 方案A rule — the AI narrates every field ("I"=AI, "You"=the answerer). English-only (zero CJK). */
const VOICE_AI_RULE = "Writing rule (this tool only): write the question, header, label, and description from the AI's point of view — \"I\" is the AI, \"You\" is the person answering (the user). Never swap them, and never call the answerer \"the user\" or any other third-person word. Keep each option self-consistent: the \"I\" in a label and the \"I\" in its description must be the same person, and the \"You\" likewise; a description may say what the answerer will do (\"You\" will …) without shifting the label's perspective. If a draft drifts — say a label from the AI's view but its description from the answerer's view — rewrite the whole block rather than patching it. This rule applies only where such pronouns actually appear — never force \"I\"/\"You\" into a question or option that does not naturally need them. This rule only governs this tool; it does not change your ordinary reply voice.";
